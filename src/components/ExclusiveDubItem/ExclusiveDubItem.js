...
import { fetchBriefData } from '../../utils/fetchBriefData'
...
const ExclusiveDubItem = ({className, item, placeholder = false, togglePreview }) => {
  ...
  return (
    <div
      className={`exclusive-dub-item ${className}`}
      onClick={() => {
        if (placeholder !== false) {
          return false
        }
    ...
    >
      <div className="item-image" onMouseOver={onHoverItem}>
        {placeholder === false && (
          <ImageRealLazyLoad
            src={getNamavaUrl(item.imageUrl || item.seriesImageUrl)}
            alt={item.caption}
          />
        )}
        {info && (
          <div className="item-hover-info">
            <div className="info-wrapper">
              <div className="info-row">
                <span>
                  {type === config.itemTypes.Series ? 'سریال' : 'فیلم'}
                </span>
                {info['year'] && (
                  <>
                    <span> - </span>
              ...
              {info['dubsType'] && info['dubsType'] !== 'None' && (
                <div className="info-row has-icon">
                  ...
                  <span>
                    {config.dubsType.ExclusiveDubs === info['dubsType']
                      ? 'دوبله نماوا'
                      : 'دوبله فارسی'}
                  </span>
                </div>
              )}
              {(info['dubsType'] == null || info['dubsType'] === 'None') &&
                info['hasPersianSubtitle'] === true && (
                  <div className="info-row has-icon">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="20"

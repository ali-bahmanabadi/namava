...
const PreviewItem = ({ id, isActive }) => {
  ...
  const afterAnimationDone = () => {
    ...
    window.scrollBy({top: (top + element.clientHeight + 30 - window.innerHeight) - window.pageYOffset, left: 0, behavior: 'smooth'})
  ...
  useEffect(() => {
    ...
    if (element.querySelector('.run-animation')) {
      element
        .querySelector('.run-animation')
        .addEventListener('animationend', afterAnimationDone)
    }
    return () => {

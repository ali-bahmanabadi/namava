  ...
  return data.type !== config.itemTypes.Series ? (
    <div className="button-box" key={`button-${data.id}`}>
      <div className="box-container">
        ...
        <div className="button-title">ورود و پخش فیلم</div>
      </div>
    </div>
  ) : (
    <div className="button-box" key={`button-${data.id}`}>
...
const getMediaDetailText = (caption, items, maxLength, keyType) => {
  let content = []
  ...
  for (let i = 0; i < maxLength && i < items.length; i++) {
    content.push(
      <a href="/" key={`text-${keyType}-${items[i][keyType + 'Id']}`}>
        {items[i][keyType + 'Name'] && items[i][keyType + 'Name']}
      </a>
    )
    content.push(
      <span
        key={`text-separator-${keyType}-${items[i][keyType + 'Id']}`}
        className="separator"
      >
        -
      </span>
...
const MovieDetail = ({ loading, data, topMedia }) => {
  ...
  return (
    <MovieDetailContainer
      imageUrl={imageUrl}
      topMedia={topMedia}
      className={`movie-detail ${topMedia === true && 'top-media'}`}
    >
      {topMedia !== true && (
        <div className="movie-image">
          {loading === false && data && data.coverLandscape && (
            <img
              src={getNamavaUrl(data.coverLandscape)}
      ...
      <div className="movie-detail-box">
        ...
        {topMedia === true && (
          <img
            src={getNamavaUrl(data.logoImageUrl)}
            alt={data.caption}
        ...
        {loading === false && data && (
          <React.Fragment>
            ...
            <div className="detail-row">
              ...
              {data.dubsType && data.dubsType !== 'None' && (
                <div className="detail-item has-icon">
                  ...
                  <span>
                    {data.dubsType === config.dubsType.ExclusiveDubs
                      ? 'دوبله نماوا'
                      : 'دوبله فارسی'}
                  </span>
                </div>
            ...
            {data.story && (
              <div className="detail-description">{data.story}</div>
            )}
            <ActionButtons
              item={data}
              {...(topMedia === true
                ? { downloadButton: true }
                : { moreButton: true })}
              defaultButton={getDefaultButton(data)}
            />
            {data.casts &&
              data.casts.length > 0 &&
              getMediaDetailText('ستارگان', data.casts, 3, 'cast')}
            {topMedia !== true && (
              <React.Fragment>
                {data.directors &&
                  data.directors.length > 0 &&
                  getMediaDetailText('کارگردان', data.director, 3, 'cast')}
                {data.categories &&

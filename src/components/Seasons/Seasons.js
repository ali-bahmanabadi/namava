...
const Seasons = ({ seasons }) => {
  ...
  useEffect(() => {
    if (
      state.loading === false &&
      state.error === false &&
      state.episodes[seasonId] === undefined
    ) {
      fetchData(
        seasonId,
        'SinglePageSeasons',
        (result) => {
          setState((state) => ({
            ...state,
            episodes: { ...state.episodes, [seasonId]: result },
            error: false,
            loading: false,
          }))
        },
    ...
    <div className="seasons-select">
      ...
      {state.active === true && (
        <ul className="seasons-list">
          {seasons.map((season) => (
            <li
              key={`seasons-list-item-${season.seasonId}`}
              onClick={() => {
